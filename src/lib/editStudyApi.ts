...
export async function updateStudy(
  ...
  const requestData: UpdateStudyRequest = {
    ...
    recruitmentMethod: data.recruitmentMethod === '선착순' ? 'FCFS' : 'APPLICATION',
    maxParticipants: parseInt(data.maxParticipants),
    schedule: data.schedule,
    curricula: data.curriculum.map((item: CurriculumItem) => item.value).join('\n'),
    qualifications: data.requirements.map((item: RequirementItem) => item.value).join('\n'),
  };
...
export const useUpdateStudyMutation = (
  ...
  return useMutation<void, HTTPError, StudyFormData>({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: STUDY_DETAIL_QUERY_KEY(studyId) });
