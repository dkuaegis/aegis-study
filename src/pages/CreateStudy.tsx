...
export default function CreateStudy({ onBack }: CreateStudyProps) {
    ...
    const updateCurriculumItem = (index: number, value: string) => {
        setCurriculum((prev) => prev.map((item, i) => (i === index ? value : item)))
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const filteredCurriculum = curriculum.filter((item) => item.trim() !== "")
        const filteredRequirements = requirements.filter((item) => item.trim() !== "")

        if (!formData.title || !formData.category || !formData.difficulty || !formData.introduction) {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="mx-auto max-w-4xl p-6">
                <div className="mb-6">
                    <h1 className="mb-2 font-bold text-3xl text-gray-900">스터디 개설하기</h1>
                    <p className="text-gray-600">새로운 스터디를 개설하고 함께 공부할 멤버들을 모집해보세요.</p>
                </div>
                        ...
                        <CardContent className="space-y-4">
                            <div>
                                ...
                                <RadioGroup
                                    ...
                                    onValueChange={(value) => handleInputChange("recruitmentMethod", value)}
