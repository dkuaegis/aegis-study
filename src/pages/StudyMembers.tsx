...
import { ArrowLeft, User, Calendar, UserX, Crown, Mail } from "lucide-react"
...
export default function StudyMembers({ studyId, onBack }: StudyMembersProps) {
    const [members, setMembers] = useState(studyMembersData[studyId as keyof typeof studyMembersData]?.members || [])

    const studyInfo = studyMembersData[studyId as keyof typeof studyMembersData]
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="mx-auto max-w-4xl p-6">
                <div className="mb-6">
                    <h1 className="mb-2 font-bold text-3xl text-gray-900">스터디원 관리</h1>
                    <p className="text-gray-600">{studyInfo.studyTitle}</p>
                </div>

                <Card className="border-gray-200">
                    <CardHeader>
                        <CardTitle className="font-semibold text-gray-900 text-lg">스터디원 목록</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gray-100">
                            {isOwner ? <Crown className="h-6 w-6 text-blue-600" /> : <User className="h-6 w-6 text-gray-600" />}
                        </div>
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <Badge
                                    variant="secondary"
                                    className={isOwner ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}
                                >
                                    {member.role}
                                </Badge>
                            </div>
                    ...
                    {!isOwner && (
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    <AlertDialogTitle>스터디원 추방</AlertDialogTitle>
                                    <AlertDialogDescription>
                                        {member.name}님을 스터디에서 추방하시겠습니까? 이 작업은 되돌릴 수 없습니다.
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                    <AlertDialogCancel>취소</AlertDialogCancel>
                                    <AlertDialogAction onClick={() => onKick(member.id)} className="bg-red-600 hover:bg-red-700">
                                        추방하기
                                    </AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
